...
namespace SlugTemplate
{
    ...
    class Plugin : BaseUnityPlugin
    {
        ...
        // 加载资源。我不知道怎么加载资源，没人说过（汗）我也不需要资源，大概。等做围巾的时候就知道需不需要了（汗
        private void LoadResources(RainWorld rainWorld)
        {
        }

        private void Room_PlaySound_SoundID_Vector2(On.Room.orig_PlaySound_SoundID_Vector2 orig, Room self, SoundID soundId, Vector2 pos)
        {
            orig(self, soundId, pos);
                    ...
                    */
                    ...
                    if (self.room.abstractRoom.creatures[i].realizedCreature != null && self.room.abstractRoom.creatures[i].realizedCreature != self.expemtObject && self.room.abstractRoom.creatures[i].realizedCreature.Submersion > 0f)
                    {
                        ...
                        if (num2 > 0f)
                        {
                            ...
                            self.room.abstractRoom.creatures[i].realizedCreature.Violence(null, null, self.room.abstractRoom.creatures[i].realizedCreature.bodyChunks[UnityEngine.Random.Range(0, self.room.abstractRoom.creatures[i].realizedCreature.bodyChunks.Length)], null, Creature.DamageType.Electric, self.damage * num2, self.damage * num2 * 240f + 30f);
                        }
                    }
                }
                self.frame++;
            ...
            if (IsMyCat.TryGet(self, out bool is_my_cat) && is_my_cat)
            {
                    ...
                    && self.onBack == null)
                {
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        ...
                        self.room.AddObject(new MouseSpark(pos + vector * 9f, self.firstChunk.vel + vector * 36f * UnityEngine.Random.value, 20f, new Color(0.7f, 1f, 1f)));
                    }
                    ...
                    && (self.canJump > 0 || self.input[0].y < 0) && self.Consious && !self.pyroJumpped && self.pyroParryCooldown <= 0f)
                {
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        Vector2 vector3 = Custom.DegToVec(360f * UnityEngine.Random.value);
                        self.room.AddObject(new MouseSpark(pos2 + vector3 * 9f, self.firstChunk.vel + vector3 * 36f * UnityEngine.Random.value, 20f, new Color(0.7f, 1f, 1f)));
                    }
                    // for (int k = 0; k < 8; k++)
                    ...
                    self.room.PlaySound(SoundID.Jelly_Fish_Tentacle_Stun, self.firstChunk.pos);
                    // self.room.PlaySound(SoundID.Zapper_Zap, pos2, 1f, 0.2f + 0.25f * UnityEngine.Random.value);
        ...
        private bool Player_GraspsCanBeCrafted(On.Player.orig_GraspsCanBeCrafted orig, Player self)
        {
            ...
            {
                Debug.Log("==== crafting results: " + self.CraftingResults());
                return (self.CraftingResults() != null); 
            }
            Debug.Log("==== not a spear OR empty stomach - GraspsCanBeCrafted");
            orig(self);
            return orig(self);
        }
                    ...
                    if (self.grasps[i] != null)
                    {
                        ...
                        if (abstractPhysicalObject.type == AbstractPhysicalObject.AbstractObjectType.Spear && !((abstractPhysicalObject as AbstractSpear).electric && (abstractPhysicalObject as AbstractSpear).electricCharge > 0))
                        {
                            if ((abstractPhysicalObject as AbstractSpear).explosive)
                            {
                                ...
                                for (int g = 0; g < 14; g++)
                                {
                                    self.room.AddObject(new Explosion.ExplosionSmoke(vector, Custom.RNV() * 5f * UnityEngine.Random.value, 1f));
                                }
                                ...
                                {
                                    Vector2 vector2 = Custom.RNV();
                                    self.room.AddObject(new Spark(vector + vector2 * UnityEngine.Random.value * 40f, vector2 * Mathf.Lerp(4f, 30f, UnityEngine.Random.value), explosiveSpear.explodeColor, null, 4, 18));
                                }
                        ...
                        if (ModManager.MSC && self.SlugCatClass == MoreSlugcatsEnums.SlugcatStatsName.Spear && (self.grasps[0] == null || self.grasps[1] == null) && num5 == -1 && self.input[0].y == 0)
                        {
                            ...
                            if (tailSpecks2.spearProg == 1f)
                            {
                                ...
                                for (int j = 0; j < 4; j++)
                                {
                                    ...
                                    self.room.AddObject(new WaterDrip(pos + Custom.RNV() * UnityEngine.Random.value * 1.5f, Custom.RNV() * 3f * UnityEngine.Random.value + vector * Mathf.Lerp(2f, 6f, UnityEngine.Random.value), false));
                                }
                                for (int k = 0; k < 5; k++)
                                {
                                    Vector2 vector2 = Custom.RNV();
